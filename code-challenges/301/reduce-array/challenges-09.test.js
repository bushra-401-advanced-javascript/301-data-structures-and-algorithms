...
const mapCurrentEvents = () => {
  const newsEvents = currentEvents.news.map(newsObj => new Event(newsObj))
  return newsEvents;
}

function Event(obj){
  this.author = obj.author;
  this.categories = obj.category || 'Not Specified';
...
const reversedString = (str) => {
  ...
  const reversedStr = stringArr.reduce((acc, letter) => {
    return letter + acc;
  }, '')
  return reversedStr;
};
