...
on several rules:
  ...
  - no subdomains, ports, etc: must be of the form name@place.com(opens in new tab), not name@sub.place.com(opens in new tab):3000
...
const validateEmail = (email) => {
  let regex = /^(\w+.\w+|\w+)@\w+(.com|.net|.org)$/g;
  if (regex.test(email)) {
    return true;
  } else { return false }
};
...
const validatePhoneNumber = (phoneNumber) => {
  let regex = /^\d{10}$|^\(\d{3}\)(\s*)\d{3}(\s|-)\d{4}$|^\d{6}-\d{4}$|^\d{3}(\s|-)\d{3}(\s|-?)\d{4}$/g;
