...
const createServer = () => {
  ...
  function greetings(request, response) {
    response.status(200).send('Hey There! Welcome to my Server.');
  }
  function briefBio(request, response) {
    const bio = 'Hi, My Name is Bushra Bilal.\nTrust Me, I Am A Full-Stack Software Developer (Hopefully =p)';
    response.status(200).send(bio);
  }
  function favFoods(request, response) {
    const mayFavFoods = ['Maqloobeh', 'Pizza', 'Pasta and Lasagna', 'Makhshi', 'Maftool'];
    response.status(200).send(mayFavFoods);
  }
};
  ...
  input.map( arr => {
    ...
    arr.forEach( element =>{
      if (typeof(element) === 'number' && !(element % 5)) {
        acceptedElements.push(Math.pow(2, element));
      }
    })
    resultsArr.push(acceptedElements);
  })
  return resultsArr;
};
